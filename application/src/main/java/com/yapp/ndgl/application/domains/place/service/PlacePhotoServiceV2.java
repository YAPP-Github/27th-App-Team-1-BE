...
public class PlacePhotoServiceV2 {
	...
	public void savePhotosIfNotExists(final String googlePlaceId) {
		...
		List<PhotoMeta> photoMetas = getPhotoMetas(place);
	...
	private List<PhotoMeta> buildPhotoMeta(final Place place, final List<PlacePhoto> existingPhotos) {
		List<PhotoMeta> photoMetas = getPhotoMetas(place);
		if (photoMetas.isEmpty()) {
			return List.of();
		}
	...
	private List<PhotoMeta> getPhotoMetas(final Place place) {
		List<PhotoMeta> photos = place.getPhotos();
		if (photos == null || photos.isEmpty()) {
			log.error("photos 조회중 에러가 발생하였습니다.");
